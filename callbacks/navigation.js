...
module.exports = (bot) => {
  ...
  // Weekly movers vs BTC
  bot.action('scan_vs_btc_1w', async (ctx) => {
    await ctx.answerCbQuery();
    const top = marketMock.getRelativeMoversVsBTC({ window: '1w', limit: 5 });
    const lines = ['📊 Top movers vs BTC (1w):', '', ...top.map((t, i) => `${i+1}. ${t.symbol}: ${(t.relative*100).toFixed(2)}%`), '', 'Tap below to open: Long ASSET / Short BTC'];
    const keyboard = Markup.inlineKeyboard(
      top.map(t => [Markup.button.callback(`${t.symbol}/BTC`, `trade_${t.symbol}_BTC`)]).concat([[Markup.button.callback('⬅️ Back', 'nav_trade')]])
    );
    await ctx.editMessageText(lines.join('\n'), keyboard);
